export const PRESETS: Record<string, string> = {
    ...
    "System Logs (500 rows)": JSON.stringify(Array.from({ length: 500 }, (_, i) => ({
        id: i + 1,
        timestamp: `2025-01-15T${String(Math.floor(i / 20) % 24).padStart(2, '0')}:${String((i * 3) % 60).padStart(2, '0')}:${String((i * 7) % 60).padStart(2, '0')}Z`,
        level: ["INFO", "WARN", "ERROR", "DEBUG"][i % 20 === 0 ? 2 : i % 10 === 0 ? 1 : i % 5 === 0 ? 3 : 0],
        service: ["api-gateway", "auth-service", "user-service", "payment-service", "inventory-service", "notification-service", "analytics-service", "search-service"][i % 8],
        host: `node-${(i % 10) + 1}.cluster.internal`,
        message: [
        ...
        userId: i % 5 === 0 ? null : `usr_${String(Math.floor(i / 3) % 1000).padStart(4, '0')}`,
        durationMs: Math.floor(Math.random() * 200) + 5,
        statusCode: [200, 200, 200, 201, 204, 400, 401, 404, 500][i % 9],
        method: ["GET", "POST", "PUT", "DELETE", "PATCH"][i % 5],
        path: ["/api/v1/users", "/api/v1/orders", "/api/v1/products", "/api/v1/auth", "/api/v1/search"][i % 5],
        region: ["us-east-1", "us-west-2", "eu-west-1", "ap-southeast-1"][i % 4],
        version: "v2.4.1",
    })), null, 2),
};
